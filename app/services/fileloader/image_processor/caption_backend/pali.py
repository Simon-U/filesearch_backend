...
# Configure logger
logger = logging.getLogger("image_processor.paligemma")
...
class PaligemmaCaptioningBackend(BaseCaptioningBackend):
    """Paligemma image captioning backend implementation"""
    
    def initialize(self):
        logger.info(f"Initializing Paligemma captioning backend with model: {self.config.caption_model}")
        ...
        logger.info(f"Paligemma captioning backend initialized in {elapsed_time:.2f} seconds")
    ...
    def analyze(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            with torch_gc_context(), torch.no_grad():
                ...
                prompt = """
                Describe this image in detail, focusing on all visible content and information. If this is a document:
                Identify the type of document (report, map, diagram, slide, form, etc.)
                Describe all visual elements present including graphs, charts, tables, or diagrams.
                Extract all readable text including titles, labels, legends, annotations, and captions.
                Report numerical data accurately and identify any color-coding or visual emphasis if present.
                Summarize the main information being communicated and its significance.
                """

                logger.info("Processing image with Paligemma model")
                ...
                output = {
                    "caption": description,
                    "confidence": 1.0,  # Paligemma doesn't provide confidence scores
                    "processing_time": {
                        "total": total_elapsed,
